...
def get_column():
	return [
		...
		_("GaureenteeCardCost") + ":Currency:90",
		_("ServiceCost") + ":Currency:90",
		_("Net") + ":Currency:90",
...
def get_count_of_serial_no(data,columns):
	...
	for d in data:
			...
			total_row[9] = flt(total_row[9]) + flt(d[9])
			total_row[10] = flt(total_row[10]) + flt(d[10])
			total_row[11] = flt(total_row[11]) + flt(d[11])
			total_row[12] = flt(total_row[12]) + flt(d[12])
			total_row[13] = flt(total_row[13]) + flt(d[13])
			total_row[14] = flt(total_row[14]) + flt(d[14])
...
CASE COALESCE(SIT.QTY,0) WHEN > 0 THEN SN.purchase_rate WHEN < 0 THEN (-1 * SN.purchase_rate) ELSE 0 END  as PurchaseRate,

SN.plate_cost AS PlateCost,

CASE COALESCE(SIT.QTY,0) WHEN > 0 THEN SIT.base_net_rate WHEN <0 THEN (-1 * SIT.base_net_rate) ELSE 0 END as TotalSelling,
