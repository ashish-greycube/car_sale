...
frappe.ui.form.on('Individual Car Stock Entry', {
	refresh: function(frm) {
		if (frm.doc.docstatus==1 && frm.doc.status=="Car Sold Out") {
			...
			if (frm.doc.payment_status=='Paid' && frm.doc.individual_car_expense_detail.length>0) {
				frm.add_custom_button(__('Expense Journal Entry'), function() {
					frappe.call({
						method: "create_expense_journal_entry",
						doc: frm.doc,
						callback: function(r) {
							if (!r.exc) {
								frm.refresh();
	...
	payment_status: function(frm) {frm.trigger("toggle_reqd_fields")},
	status: function(frm) {frm.trigger("toggle_reqd_fields")},
	reservation_status: function(frm) {frm.trigger("toggle_reqd_fields")},
